...
original_data = pd.read_csv("Global_Landslide_Catalog_Export.csv",
                            index_col=0)  # We first put the data into a dataframe called original data
original_data.to_csv('land_original_data.csv')
print(original_data)
...
'''
selected_data = ['landslide_category', 'landslide_trigger', 'landslide_size',
                 'landslide_setting', 'fatality_count','injury_count']


original_data=original_data[selected_data]


print("\n-----------Original data summary (missing values)-------------\n")
print(original_data.isna().sum())
original_data['landslide_setting']=original_data['landslide_setting'].fillna('unknown')
original_data.to_csv('land_original_data_v2.csv')
print(original_data.isna().sum())
...
complete_data.to_csv('land_complete_data.csv')
print(complete_data)
...
print(original_data)
print(incomplete_data)
incomplete_data.to_csv("land_incomplete_data.csv")
print(complete_data)
print("\n-----------Data description-------------\n")
...
omit_data.to_csv('land_omit_data.csv')
print(omit_data)
